...
module.exports = {
    ...
    execute: (Client, interaction) => {
        ...
        if (chList.length >= 3) return interaction.reply({
            embeds: [{
                title: 'Erreur',
                color: 0xED4245,
                cescription: 'Vous avez deja atteint le maximum de tickets (3 tickets max) !'
            }], ephemeral: true
        })
        ...
        interaction.guild.channels.create({
            ...
            channel.send({
                embeds: [{
                    title: 'Voici votre ticket',
                    description: 'Decrivez votre problème/demande le temps qu\'un membre du staff vous prenne en charge.\nVotre id : ' + interaction.user.id,
                ...
                .then(() => {
                    interaction.reply({
                        embeds: [{
                            title: 'Voici votre ticket ' + interaction.member.displayName,
                            color: config.application.color,
                            description: 'Votre ticket à bien été crée dans le channel <#' + channel.id + ">",
                        }], ephemeral: true
                    }).then(() => {
